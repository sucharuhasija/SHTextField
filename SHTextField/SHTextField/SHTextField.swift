...
###################################################################################
#                                                                                 #
#  Step 1a) Change the Type Of SHTextField in Identity Inspector Of The textField  #
#          to change the class of Textfield to be SHTextField                      #
#                       OR                                                        #
#  Step 1b) Define Programmatically a Textfield of Type SHTextField               #
#                                                                                 #
#  Step 2. Define The Method   (Neccessary)                                       #
#                                                                                 #
#         self.itextFeild.initilizeSHTextFieldWithEmailHavingDefaultSettings()     #
#                                                                                 #
#  Step3. SetAppropriate Labels in method (Optional)                              #
#                                                                                 #
#   self.SHTextField.SHTextFieldSetIntialPlaceholderText( primaryPlaceholder        #
...
case UserDefined      = 6
//case MobileNumber     = 7
// case SHTextFieldTypeGoogleAddress = 8



}

protocol SHTextFieldProtocol:class
{
/*
...
1. Add the Delegate method  as SHTextFieldProtocol
2. Add Delegate as self.SHTextField.delegateOfSHTextField = self
...
public class SHTextField: USHTextField ,USHTextFieldDelegate{

    
    // Delegate For the Custom User Actions
     weak var delegateOfSHTextField:SHTextFieldProtocol?
    /*
...
// MARK:  Initilizers
    ...
    public func initilizeSHTextFieldWithEmailHavingDefaultSettings()
    {
   
        self.initilizeNormalSupportAutoComple(true)
        self.type = SHTextFieldType.Email
        self.SHTextFieldSetIntialPlaceholderText("Email", withtypingPlaceholderText: "XXX@XX.XX", withWarningPlaceholder: "N/A", andCompletionPlaceholer: "😊")
       self.keyboardType = UIKeyboardType.EmailAddress
    
    }
    func initilizeSHTextFieldWithSimplePasswordHavingRangeFrom(min:Int ,to max:Int )
    {
        
        
        self.initilizeNormalSupportAutoComple(false)
        self.type = SHTextFieldType.SimplePassword
        self.minimumCharacterRequired = min
        self.maximumCharacterAllowed = max
        self.contentOffset = 4
        self.keyboardType = .Default
        self.secureTextEntry = true
        self.SHTextFieldSetIntialPlaceholderText("Enter Password", withtypingPlaceholderText: "\(min)-\(max) characters", withWarningPlaceholder: "N/A", andCompletionPlaceholer: "")
    
    }
    func initilizeSHTextFieldWithStrictPasswordHavingRangeFrom(min:Int ,to max:Int )
    {
    
            self.initilizeNormalSupportAutoComple(false)
            self.type = SHTextFieldType.StrictPassword
            self.minimumCharacterRequired = min
            self.maximumCharacterAllowed = max
            self.keyboardType = .Default
            self.contentOffset = 4
            self.secureTextEntry = true
            self.SHTextFieldSetIntialPlaceholderText("Enter Password", withtypingPlaceholderText:  "\(min)-\(max) characters", withWarningPlaceholder: "", andCompletionPlaceholer: "")
        
    }
    func initilizeSHTextFieldWithDateOfBirthHavingAgeLimitFrom(min:Int ,to max:Int )
    {
        self.initilizeNormalSupportAutoComple(true)
        self.type = SHTextFieldType.DateOfBirth
        self.minimumAge = min
        self.MaximumAge = max
        self.keyboardType = .NumberPad
        self.SHTextFieldSetIntialPlaceholderText("Enter Age", withtypingPlaceholderText: "DD/MM/YYYY", withWarningPlaceholder: "N/A", andCompletionPlaceholer: "😊")
    
    }
    
    
    
    public  func intitlizeSHTextFieldWithTypeUserDefinedMustAddDelegateWithAllowedCharacterRangeFrom(min:Int ,to max:Int ,andSuggestionArray suggestion:[String] )
    {
        self.initilizeNormalSupportAutoComple(true)
        self.type = SHTextFieldType.UserDefined
        self.minimumCharacterRequired = min
        self.maximumCharacterAllowed = max
        self.userSuggestions = suggestion
        self.keyboardType = .Default
        self.SHTextFieldSetIntialPlaceholderText("User defined", withtypingPlaceholderText: "Typing....", withWarningPlaceholder: "N/A", andCompletionPlaceholer: "😊")
    }
    ...
    private func initilizeNormalSupportAutoComple(value:Bool)
    {
        ...
        NSNotificationCenter.defaultCenter().addObserver(self, selector:#selector(SHTextField.tfTextDidChange), name: USHTextFieldTextDidChangeNotification, object: self)
        NSNotificationCenter.defaultCenter().addObserver(self, selector:#selector(SHTextField.CheckValidityOfText),name: USHTextFieldTextDidEndEditingNotification, object: self)
        NSNotificationCenter.defaultCenter().addObserver(self, selector:#selector(SHTextField.tfStartTyping), name: USHTextFieldTextDidBeginEditingNotification, object: self)
        ...
        self.parkLabel?.addTarget(self, action: #selector(SHTextField.autoCompleteText), forControlEvents: UIControlEvents.TouchUpInside)
     ...
     public func SHTextFieldSetIntialPlaceholderText(initialPlaceholderText:String,withtypingPlaceholderText secondaryPlaceholder:String,withWarningPlaceholder warning:String ,andCompletionPlaceholer completionPlaceholder:String)
    {
    ...
    
    {
        ...
        UIView.animateWithDuration(0.2, delay: 0.0, usingSpringWithDamping: 0.9, initialSpringVelocity: 0, options: .CurveLinear, animations: { () -> Void in
                ...
                switch(self.type!)
                {
                    ...
                    case SHTextFieldType.Email , .DateOfBirth:
            
                        self.bringSubviewToFront(self.parkLabel!)
                        let autoCompletetext = self.autoCompleteTextForSHTextFieldOfType( self.type!, withTextForAutoCompletion: self.text!)
                    ...
                    self.bringSubviewToFront(self.parkLabel!)
                    let autoCompletetext = self.autoCompleteTextForSHTextFieldOfType( .UserDefined, withTextForAutoCompletion: self.text!)
...
// MARK: Check Validity When user Ended Typing
    func CheckValidityOfText()
    {
        ...
        case .SimplePassword:
            ...
            if self.type == SHTextFieldType.SimplePassword
            {
        ...
        case .UserDefined:
            ...
            assert(delegateOfSHTextField != nil, "SHTextField.delegateOfSHTextField must define")
            
            if (delegateOfSHTextField!.isValidOnCondition(self.text!))
            {
...
//MARK: Delegate Method to prevent Typing to have limit users to type within limit of characters
    ...
    public func textField(textField: USHTextField, shouldChangeCharactersInRange range: NSRange, replacementString string: String) -> Bool {
    ...
    private func autoCompleteTextForSHTextFieldOfType(type:SHTextFieldType , withTextForAutoCompletion text:String ) -> String
    {
    
            /*
                    IT CHECKS THE LAST COMPONENET OF THE TEXT ENTERED IN THE SHTextField SO THAT  WE CAN TEST THAT STRING TO AVAIL AUTOCOMPLETE FUNCTIONALITY
...
// MARK: Customize the Text Inside TextField
    ...
    // Test Valdity of the date inside SHTextField
